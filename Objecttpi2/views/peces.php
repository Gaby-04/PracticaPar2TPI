<?php
require_once "models\animal.php";
?>




<h1 class="text-center">Peces</h1>
    <div class="row row-cols-1 row-cols-md-3 g-4 mb-3">
    <div class="col">
        <div class="card h-100">
        <img src="img/pez1.jpg" class="card-img-top" style="height: 200px;">
        <div class="card-body">
            <h5 class="card-title"><?php echo Dephine::Nombre() ?></h5>
            <p class="card-text"><?php echo Dephine::descripcion() ?></p>
            <audio src="sonidos/torogoz.mp3" controls=""></audio>
        </div>
        </div>
    </div>
    <div class="col">
        <div class="card h-100">
        <img src="img/pez2.jpg" class="card-img-top" style="height: 200px;">
        <div class="card-body">
            <h5 class="card-title"><?php echo Tiburon::Nombre() ?></h5>
            <p class="card-text"><?php echo Tiburon::descripcion() ?></p>
            <audio src="sonidos/pingui.mp3" controls=""></audio>
        </div>
        </div>
    </div>
    <div class="col">
        <div class="card h-100">
        <img src="img/pez3.jpg" class="card-img-top" style="height: 200px;">
        <div class="card-body">
            <h5 class="card-title"><?php echo Estrella_mar::Nombre() ?></h5>
            <p class="card-text"><?php echo Estrella_mar::descripcion() ?></p>
            <audio src="sonidos/avestruz.mp3" controls=""></audio>
        </div>
        </div>
    </div>
    
    </div>
</div>